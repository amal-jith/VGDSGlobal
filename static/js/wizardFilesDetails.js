document.addEventListener('DOMContentLoaded', () => {
  ...
  fileInput.addEventListener('change', (e) => {
    if (e.target.files.length > 0) {
      ...
      window.presentationWizardPresentationFile = uploadedFile;
...
function updateEstimatedPriceStep4() {
  ...
  console.log("🔍 Loaded estimatedText from localStorage:", estimated);
  if (estimated && priceRangeDisplay) {
    priceRangeDisplay.textContent = estimated;
    console.log("✅ Updated DOM with estimatedText:", estimated);
  } else {
    priceRangeDisplay.textContent = "$0";
    console.log("❌ No estimatedText found. Defaulting to $0");
  }
}
  ...
  nextBtn?.addEventListener('click', () => {
      ...
      // ✅ Update estimated price on payment step
      setTimeout(() => {
        const wizardData = JSON.parse(localStorage.getItem('presentationWizard')) || {};
        const estimated = wizardData?.delivery?.estimatedText;
        const paymentPriceEl = document.querySelector('#step-payment .price-range');

        if (paymentPriceEl && estimated) {
          paymentPriceEl.textContent = estimated;
          console.log("✅ Updated estimated price on payment page:", estimated);
        } else {
          paymentPriceEl.textContent = "$0";
          console.warn("⚠️ Estimated price missing, fallback to $0");
        }
