...
slideInput.addEventListener('blur', () => {
  ...
  saveDeliveryData(); // Save latest data first
  ...
  // THEN update price after DOM is visible
  setTimeout(() => {
    const priceEl = document.querySelector('#step-files-details .price-range');
    const wizardData = JSON.parse(localStorage.getItem('presentationWizard')) || {};
    if (priceEl && wizardData?.delivery?.estimatedText) {
      priceEl.textContent = wizardData.delivery.estimatedText;
      console.log("✅ Price updated after transition:", wizardData.delivery.estimatedText);
    } else {
      console.warn("⚠️ Could not find estimatedText or price element.");
    }
  }, 100);
});
